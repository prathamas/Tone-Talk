...
const SideBar = () => {
  ...
  return (
    <div className={`bg-[#282142]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? "max-md:hidden":""}`}>
        <div className='pb-5'>
            <div className='flex justify-between items-center'>
                <img src={assets.logo} alt="logo" className='max-w-40 max-h-40'/>
        ...
        <div className='flex flex-col'>
            {filteredUsers.map((user,index)=>(
                ...
                key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id  && "bg-[#282142]/50"} `}>
                    <img src={user?.profilePic || assets.arrow_icon} alt="profile pic" 
